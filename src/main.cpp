...
const float longitude = 77.566895;
...
#define DATABASE_URL                                                           \
  "https://"                                                                   \
  "esp32-theft-detection-default-rtdb.asia-southeast1.firebasedatabase.app"
...
#define FIREBASE_AUTH                                                          \
  "AIzaSyBc1-TjprtzvD0S_AcNNnO3xhs-c1pBYVU" // Use API key as auth token
...
const int firebaseUpdateInterval =
  ...
  if (currentDifference > theftThreshold) {
    ...
    if (!theftDetected) {
      ...
      if (millis() - theftStartTime >= debounceTime) {
        theftDetected = true;
        handleTheftDetected();
        // No automatic power cutoff - this will now be done manually via
        // dashboard
      }
    }
    ...
    if (theftDetected) {
      ...
      if (theftRemovalTime == 0) {
        ...
        if (WiFi.status() == WL_CONNECTED && Firebase.ready()) {
          Firebase.setFloat(fbdo, "/theft_detection/difference",
                            currentDifference);
          Firebase.setInt(fbdo, "/theft_detection/theft_removal_start",
                          millis());
        }
      }
      ...
      if (millis() - theftRemovalTime >= restoreDelay) {
        ...
        theftStartTime = 0;
        Serial.println(
            "Theft condition resolved - but power remains in current state");

        // Immediately update Firebase when theft is no longer detected
        if (previousTheftState && WiFi.status() == WL_CONNECTED &&
            Firebase.ready()) {
          ...
          Firebase.setInt(fbdo, "/theft_detection/alert/resolved_time",
                          millis());
        }
      }
    } else {
...
void handleNoTheft() { Serial.println("No Theft"); }
...
float measureCurrent(int sensorPin, float offsetVoltage) {
  long sumSensorValue = 0;
  int actualSamples =
      250; // Reduced from 500 to 250 samples for faster measurement
  ...
  // Add fixed calibration offset of 0.15A
  current += 0.15;
...
void sendDataToFirebase() {
  ...
  if (Firebase.ready() &&
      (lastTheftState != theftDetected ||
       millis() - lastFirebaseUpdate >= firebaseUpdateInterval)) {
    // Update sensor data
  ...
  if (Firebase.getInt(fbdo, POWER_STATE_PATH)) {
    ...
    if (desiredState != currentState) {
      ...
      if (desiredState == POWER_OFF) {
        ...
        // Let the actual current measurements determine if theft is still
        // happening theftDetected remains unchanged until current measurements
        // confirm
